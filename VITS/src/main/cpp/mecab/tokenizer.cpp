...
template Node* Tokenizer<Node, Path>::lookup<true>(
    ...
    Allocator<Node, Path> *,
    Lattice *) const;
template bool Tokenizer<Node, Path>::open(const Param &, AssetJNI* asjni);
...
//template bool Tokenizer<LearnerNode, LearnerPath>::open(const Param &, AssetJNI asjni);

template <typename N, typename P>
Tokenizer<N, P>::Tokenizer()
    : dictionary_info_freelist_(4),
      dictionary_info_(nullptr),
...
template <typename N, typename P>
bool Tokenizer<N, P>::open(const Param &param, AssetJNI* asjni) {
  close();

  const std::string prefix = param.template get<std::string>("dicdir");

  CHECK_FALSE(unkdic_.open(create_filename
                                 (prefix, UNK_DIC_FILE).c_str(),asjni))
      << unkdic_.what();
  CHECK_FALSE(property_.open(param, asjni)) << property_.what();

  auto *sysdic = new Dictionary;

  CHECK_FALSE(sysdic->open
                    (create_filename(prefix, SYS_DIC_FILE).c_str(),asjni))
      << sysdic->what();
  ...
  if (!userdic.empty()) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      CHECK_FALSE(d->open(dicfile[i],asjni)) << d->what();
  ...
  for (size_t i = 0; i < property_.size(); ++i) {
    ...
    unk_tokens_.emplace_back(token, size);
  }
