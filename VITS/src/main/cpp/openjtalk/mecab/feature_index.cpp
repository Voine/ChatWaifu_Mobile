...
bool EncoderFeatureIndex::open(const Param &param,AssetJNI* asjni) {
  return openTemplate(param);
}

bool DecoderFeatureIndex::open(const Param &param, AssetJNI* asjni) {
  const std::string modelfile = param.get<std::string>("model");
  // open the file as binary mode again and fallback to text file
  if (!openBinaryModel(param, asjni)) {
    std::cout << modelfile
...
bool DecoderFeatureIndex::openFromArray(const char *begin, const char *end) {
  ...
  const auto file_size = static_cast<size_t>(end - begin);
...
bool DecoderFeatureIndex::openBinaryModel(const Param &param, AssetJNI* asjni) {
  const std::string modelfile = param.get<std::string>("model");
  CHECK_DIE(mmap_.open(modelfile.c_str(),asjni)) << mmap_.what();
...
bool FeatureIndex::convert(const Param &param,
  ...
  output->clear();
  auto size = static_cast<unsigned int>(dic.size());
