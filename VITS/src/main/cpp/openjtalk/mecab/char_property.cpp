...
bool CharProperty::open(const Param &param, AssetJNI* asjni) {
  const std::string prefix   = param.get<std::string>("dicdir");
  const std::string filename = create_filename(prefix, CHAR_PROPERTY_FILE);
  return open(filename.c_str(), asjni);
}

bool CharProperty::open(const char *filename, AssetJNI* asjni) {
  std::ostringstream error;
  CHECK_FALSE(cmmap_->open(filename, asjni, "r"));
